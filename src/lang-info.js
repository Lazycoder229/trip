...
const DOCS = {
  ...
  fn:       ['keyword', '`fn name(params):` _(indented body)_', 'Declares a function.'],
  return:   ['keyword', '`return value`', 'Returns a value from the enclosing function.'],
  if:       ['keyword', '`if cond:` _(indented body)_', 'Conditional branch.'],
  else:     ['keyword', '`else:` _(indented body)_', 'Fallback branch for an `if`.'],
  elif:     ['keyword', '`elif cond:` _(indented body)_', 'Additional conditional branch.'],
  while:    ['keyword', '`while cond:` _(indented body)_', 'Loops while the condition is true.'],
  for:      ['keyword', '`for item in iterable:` _(indented body)_', 'Iterates over an iterable.'],
  in:       ['keyword', 'used with `for`', 'Specifies the iterable in a for-loop.'],
  class:    ['keyword', '`class Name:` _(indented body)_', 'Declares a class.'],
  ...
  out:    ['builtin', '`out(...values)`', 'Writes value(s) to stdout, joined with no separator, plus a trailing newline.'],
  outn:  ['builtin', '`outn(...values)`', 'Writes value(s) to stdout, joined with a space, plus a trailing newline.'],
  inp:    ['builtin', '`int()`', 'Placeholder — synchronous stdin input is not yet implemented.'],
